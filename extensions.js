...
document.addEventListener('DOMContentLoaded', function () {
  // Function to add the reset icon to the chat input
  function addResetIcon() {
    // Select the chat input area
    const chatInput = document.querySelector('.vfrc-chat-input');

    // Proceed only if the chat input area exists
    if (chatInput) {
      // Check if the reset icon already exists
      if (!chatInput.querySelector('.reset-icon')) {
        // Create a new div element for the reset icon
        const resetIconContainer = document.createElement('div');
        resetIconContainer.classList.add('reset-icon');

        // Insert your SVG icon into the container
        resetIconContainer.innerHTML = `
        ...
        // Append the reset icon to the chat input area
        chatInput.appendChild(resetIconContainer);

        // Add a click event listener to the reset icon
        resetIconContainer.addEventListener('click', function () {
          // Reset the chat when the icon is clicked
          if (window.voiceflow && window.voiceflow.reset) {
            window.voiceflow.reset();
          } else if (window.voiceflow && window.voiceflow.chat && window.voiceflow.chat.reset) {
            window.voiceflow.chat.reset();
          } else if (window.VoiceflowAssistant && window.VoiceflowAssistant.reset) {
            window.VoiceflowAssistant.reset();
          } else {
            // If the reset function is not available, reload the page as a fallback
  ...
  // Call the function to add the reset icon
  addResetIcon();

  // Optional: If the chat widget dynamically updates, observe for changes and re-add the icon if necessary
  const observer = new MutationObserver(addResetIcon);
